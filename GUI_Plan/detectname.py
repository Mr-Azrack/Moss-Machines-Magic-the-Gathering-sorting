...
def _get_reader():
    global _reader
    if _reader is None:
        # Force CPU and silence easyocr init prints
        os.environ.setdefault('CUDA_VISIBLE_DEVICES', '')
        try:
            f1, f2 = io.StringIO(), io.StringIO()
            with contextlib.redirect_stdout(f1), contextlib.redirect_stderr(f2):
...
reader = None  # replaced by _get_reader()
...
def find_text(frame, card_contour):
    try:
        ...
        for img in processed_images:
            ...
            result = _get_reader().readtext(scaled, detail=0)
        ...
        pass  # suppressed OCR error print
        return None
